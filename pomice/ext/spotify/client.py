...
class Client:
    ...
    async def search(
        ...
        *, query: str,
        ...
        if not (match := SPOTIFY_URL_REGEX.match(query)):
        ...
        if spotify_type == 'track':
            ...
            processes = [_fetch_async(url, count) for count, url in enumerate(
                         urls[:9] if not full else urls, start=1)]
            ...
            cls._create_partial(urls[9:] if not full else [])
            return cls
